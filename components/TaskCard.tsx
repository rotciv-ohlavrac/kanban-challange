"use client";

import React from "react";
import { useSortable } from "@dnd-kit/sortable";
import { CSS } from "@dnd-kit/utilities";
import { cva } from "class-variance-authority";
import { Task } from "../types";
import { UnifiedGitHubService } from "../services/unifiedGitHubService";
import { useTranslations } from "next-intl";

interface TaskCardProps {
  task: Task;
  onClick: () => void;
}

// Story Points Variants
const storyPointsStyles = cva(
  [
    "ml-2",
    "flex-shrink-0",
    "w-8",
    "h-8",
    "rounded-full",
    "flex",
    "items-center",
    "justify-center",
    "text-xs",
    "font-bold",
  ],
  {
    variants: {
      points: {
        zero: ["bg-gray-100", "text-gray-700"],
        low: ["bg-green-100", "text-green-700"], // 1-3 points
        medium: ["bg-yellow-100", "text-yellow-700"], // 4-8 points
        high: ["bg-red-100", "text-red-700"], // 9+ points
      },
    },
    defaultVariants: {
      points: "zero",
    },
  }
);

// User Role Variants
const userRoleStyles = cva(
  ["px-2", "py-1", "text-xs", "rounded-full", "font-medium"],
  {
    variants: {
      role: {
        developer: ["bg-blue-100", "text-blue-700"],
        designer: ["bg-purple-100", "text-purple-700"],
        pm: ["bg-green-100", "text-green-700"],
        qa: ["bg-orange-100", "text-orange-700"],
        default: ["bg-gray-100", "text-gray-700"],
      },
    },
    defaultVariants: {
      role: "default",
    },
  }
);

export const TaskCard: React.FC<TaskCardProps> = ({ task, onClick }) => {
  const tPoints = useTranslations("storyPoints");
  const tUser = useTranslations("user");
  const {
    attributes,
    listeners,
    setNodeRef,
    transform,
    transition,
    isDragging,
  } = useSortable({ id: task.id });

  const githubService = new UnifiedGitHubService();

  const getPointsVariant = (points: number) => {
    if (points === 0) return "zero";
    if (points <= 3) return "low";
    if (points <= 8) return "medium";
    return "high";
  };

  const getRoleVariant = (role: string) => {
    return role === "developer" ||
      role === "designer" ||
      role === "pm" ||
      role === "qa"
      ? (role as "developer" | "designer" | "pm" | "qa")
      : "default";
  };

  const style = {
    transform: CSS.Transform.toString(transform),
    transition,
    opacity: isDragging ? 0.5 : 1,
  };

  // Render functions for better performance
  const renderStoryPoints = () => {
    if (task.storyPoints <= 0) return null;

    return (
      <div
        className={storyPointsStyles({
          points: getPointsVariant(task.storyPoints),
        })}
      >
        {task.storyPoints}
      </div>
    );
  };

  const renderAssignedUser = () => {
    if (!task.assignedTo) return null;

    return (
      <div className="flex items-center gap-2 mb-3">
        {task.assignedTo.avatar && (
          <img
            src={task.assignedTo.avatar}
            alt={task.assignedTo.name}
            className="w-6 h-6 rounded-full"
          />
        )}
        <span className="text-xs font-medium text-gray-700 flex-1 truncate">
          {task.assignedTo.name}
        </span>
        <span
          className={userRoleStyles({
            role: getRoleVariant(task.assignedTo.role),
          })}
        >
          {tUser(`roles.${task.assignedTo.role}`)}
        </span>
      </div>
    );
  };

  const renderGitHubPR = () => {
    if (!task.githubPR) return null;

    return (
      <div className="mt-3 p-2 bg-gray-50 rounded-md border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm">
              {githubService.getStatusIcon(task.githubPR.status)}
            </span>
            <span className="text-xs font-medium text-gray-700">
              #{task.githubPR.number}
            </span>
            <span
              className={`px-2 py-1 text-xs font-medium rounded-full ${githubService.getStatusColor(
                task.githubPR.status
              )}`}
            >
              {task.githubPR.status}
            </span>
          </div>
          <a
            href={task.githubPR.url}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            className="text-blue-600 hover:text-blue-800 text-xs"
            title="Ver PR no GitHub"
          >
            GitHub
          </a>
        </div>
        <p className="text-xs text-gray-600 mt-1 truncate">
          {task.githubPR.branch}
        </p>
      </div>
    );
  };

  const renderUpdateDate = () => {
    if (task.updatedAt.getTime() === task.createdAt.getTime()) return null;

    return (
      <span>Atualizado em {task.updatedAt.toLocaleDateString("pt-BR")}</span>
    );
  };

  return (
    <div
      ref={setNodeRef}
      style={style}
      {...attributes}
      {...listeners}
      onClick={onClick}
      className="bg-white border border-gray-200 rounded-lg p-4 mb-3 cursor-pointer hover:shadow-lg transition-all duration-200 hover:border-blue-400 hover:scale-[1.02]"
    >
      <div className="flex justify-between items-start mb-2">
        <h3 className="font-semibold text-gray-900 line-clamp-2 flex-1">
          {task.title}
        </h3>
        {renderStoryPoints()}
      </div>

      <p className="text-sm text-gray-600 line-clamp-3 mb-3">
        {task.description}
      </p>

      {renderAssignedUser()}

      {renderGitHubPR()}

      <div className="mt-3 flex justify-between items-center text-xs text-gray-500">
        <span>Criado em {task.createdAt.toLocaleDateString("pt-BR")}</span>
        {renderUpdateDate()}
      </div>
    </div>
  );
};
